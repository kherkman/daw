// midi-guitar.js
window.CustomAudioEffect = class MidiGuitarEffect {
    constructor(audioCtx) {
        ...
        // Piilotetut filtterit kitaralle
        ...
        // Kitaran 6 kieltä ja niiden tila (Monofoninen kielilogiikka)
        this.numStrings = 6;
        this.stringStates = Array.from({ length: this.numStrings }, () => ({
            heldNotes: [], 
            activeVoice: null,
            lastVelocity: 100 // Tallennetaan voimakkuus pull-offia varten
        }));
        
        this.stringOffset = 0;
        this.styleTriggers = [72, 74, 76];
        this.defaultTunings = [40, 45, 50, 55, 59, 64]; 
    ...
    async loadDefaultSamples() {
        const files = ['guitar/guitar1.wav', 'guitar/guitar2.wav', 'guitar/guitar3.wav'];
        for (let i = 0; i < files.length; i++) {
            try {
                ...
                console.warn(`Midi-guitar: Ei voitu ladata oletussamplea ${files[i]}`);
            }
        }
    }
    ...
    noteOn(midiNote, velocity, forceStringIdx = -1, isExternalMidi = false) {
        ...
        state.lastVelocity = velocity; // Päivitetään muistiin pull-offia varten
        const now = this.ctx.currentTime;
        const peakGain = Math.pow(velocity / 127, 2);

        if (!state.activeVoice) {
            // Normaali näppäily (Pick)
            ...
            // HAMMER-ON! 
            // Feidataan vanha ääni nopeasti pois...
            ...
            // ...ja käynnistetään uusi ääni uudella nuotilla.
            ...
            newVca.gain.linearRampToValueAtTime(peakGain, now + 0.02); // Nopea isku ylös

            newSource.connect(newVca);
            newVca.connect(this.wet);
            
            // TÄRKEÄÄ: Ohitetaan samplen ensimmäiset 50ms (plektran isku), jotta saadaan puhdas Legato-soundi!
    ...
    noteOff(midiNote, isExternalMidi = false) {
        ...
        for (let s = 0; s < this.numStrings; s++) {
            ...
            if (idx !== -1) {
                // Tarkistetaan katkaistiinko nimenomaan se nuotti joka soi
                ...
                if (state.heldNotes.length === 0) {
                    // Kaikki sormet ylhäällä -> vaimenna kieli
                    if (state.activeVoice) {
                        ...
                        voice.source.stop(now + 1.0);
                        state.activeVoice = null;
                    }
                } else if (wasHighest && state.activeVoice) {
                    // PULL-OFF! Ylin sormi nousi, mutta alempi on yhä painettuna.
                    const newHighest = state.heldNotes[state.heldNotes.length - 1];
                    const oldVoice = state.activeVoice;

                    // Katkaistaan vanha ääni...
                    oldVoice.vca.gain.cancelScheduledValues(now);
                    oldVoice.vca.gain.setValueAtTime(oldVoice.vca.gain.value, now);
                    oldVoice.vca.gain.linearRampToValueAtTime(0, now + 0.02);
                    oldVoice.source.stop(now + 0.05);

                    // ...ja käynnistetään alemman nuotin ääni.
                    const newSource = this.ctx.createBufferSource();
                    newSource.buffer = this.buffers[this.currentStyle];
                    newSource.playbackRate.value = Math.pow(2, (newHighest - this.baseMidi) / 12);
                    ...
                    // Pull-off on hieman pehmeämpi kuin näppäily
    ...
    updateVisuals(stringIdx) {
        ...
        const frets = stringRow.querySelectorAll('.guitar-fret');
    ...
    initKeyboardListeners() {
        const baseKeyMap = {
            'c':{r:0, f:0}, 'v':{r:0, f:1}, 'b':{r:0, f:2}, 'n':{r:0, f:3}, 'm':{r:0, f:4},
            'f':{r:1, f:0}, 'g':{r:1, f:1}, 'h':{r:1, f:2}, 'j':{r:1, f:3}, 'k':{r:1, f:4},
            't':{r:2, f:0}, 'y':{r:2, f:1}, 'u':{r:2, f:2}, 'i':{r:2, f:3}, 'o':{r:2, f:4},
            '6':{r:3, f:0}, '7':{r:3, f:1}, '8':{r:3, f:2}, '9':{r:3, f:3}, '0':{r:3, f:4}
        };
        ...
        window.addEventListener('keydown', async (e) => {
            ...
            if (key === '5') {
                this.stringOffset = Math.min(2, this.stringOffset + 1);
                this.renderFretboardLabels();
                return;
            }
            if (key === '4') {
                this.stringOffset = Math.max(0, this.stringOffset - 1);
                this.renderFretboardLabels();
                return;
            }

            const map = baseKeyMap[key];
            if (map) {
                this.keyStates[key] = true;
                const activeString = map.r + this.stringOffset;
                const note = this.currentTunings[activeString] + map.f;
                this.noteOn(note, 100, activeString);
            }
        });
    ...
    renderFretboardLabels() {
        ...
        for (let s = 0; s < 6; s++) {
            const frets = this.uiElements.strings[s].querySelectorAll('.guitar-fret');
            frets.forEach(fret => {
                const hint = fret.querySelector('.key-hint');
        ...
        for (let s = 0; s < 6; s++) {
            ...
            for (let f = 0; f < 5; f++) {
                ...
                const rowIdx = s - this.stringOffset;
                if (rowIdx >= 0 && rowIdx <= 3) {
                    const keyHint = frets[f].querySelector('.key-hint');
                    if (keyHint) keyHint.innerText = `[${keyMapGrid[rowIdx][f]}]`;
                }
            }
        }
    }
    ...
    setState(state) {
        ...
        if (state.stringOffset !== undefined) this.stringOffset = state.stringOffset;
    ...
    renderUI(containerElement) {
        ...
        const styleId = 'fx-guitar-styles';
        if (!document.getElementById(styleId)) {
            const style = document.createElement('style');
            style.id = styleId;
            style.textContent = `
                .guitar-panel { background: #111; padding: 15px; border-radius: 8px; border: 1px solid #333; box-shadow: inset 0 0 20px rgba(0,0,0,0.5); font-family: monospace; display: flex; flex-direction: column; gap: 15px; }
                .guitar-row { display: flex; justify-content: space-between; align-items: flex-end; gap: 15px; flex-wrap: wrap; }
                ...
                .btn-neon:hover { background: rgba(0, 204, 255, 0.1); box-shadow: 0 0 5px rgba(0,204,255,0.4); }
                ...
                .fretboard-row { display: flex; height: 32px; position: relative; align-items: center; }
                
                @keyframes string-vibrate { 0% { transform: translateY(-50%); } 25% { transform: translateY(calc(-50% - 1.5px)); } 50% { transform: translateY(-50%); } 75% { transform: translateY(calc(-50% + 1.5px)); } 100% { transform: translateY(-50%); } }
                ...
                .row-0 .string-static, .row-0 .string-vibrating { height: 5px; }   /* E2 */
                .row-1 .string-static, .row-1 .string-vibrating { height: 4px; }   /* A2 */
                .row-2 .string-static, .row-2 .string-vibrating { height: 3px; }   /* D3 */
                .row-3 .string-static, .row-3 .string-vibrating { height: 2px; }   /* G3 */
                .row-4 .string-static, .row-4 .string-vibrating { height: 1.5px; } /* B3 */
                .row-5 .string-static, .row-5 .string-vibrating { height: 1px; }   /* E4 */

                /* Frets */
                .guitar-fret { flex: 1; height: 100%; border-right: 3px solid #777; position: relative; z-index: 2; display: flex; justify-content: center; align-items: center; cursor: pointer; user-select: none; box-sizing: border-box; background: rgba(0,0,0,0.2); }
                .guitar-fret:hover { background: rgba(0,204,255,0.15); }
                
                /* Satula (Nut) */
                .nut-fret { border-right: 6px solid #d4d4d4 !important; background: rgba(255,255,255,0.03); }

                /* Active Note Dot */
                .guitar-fret::after { content: attr(data-note); position: absolute; width: 20px; height: 20px; background: var(--fx-color); border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 9px; font-weight: bold; color: #000; opacity: 0; transition: opacity 0.1s; box-shadow: 0 0 10px var(--fx-color); pointer-events: none; }
                .guitar-fret.active::after { opacity: 1; }
                ...
                .key-hint { position: absolute; top: 2px; font-size: 8px; color: #ff0077; display: none; pointer-events: none; background: rgba(0,0,0,0.6); padding: 1px 3px; border-radius: 2px;}
                .show-keys .key-hint:not(:empty) { display: block; }
                ...
                .knob-svg { width: 100%; height: 100%; transform: rotate(135deg); filter: drop-shadow(0 0 3px rgba(0,204,255,0.2));}
        ...
        containerElement.innerHTML = `
            <div style="margin-bottom: 10px; color: var(--fx-color); font-weight: bold; text-align: center; letter-spacing: 4px; font-size: 14px; text-shadow: 0 0 10px rgba(0,204,255,0.5); font-family: monospace;">MIDI GUITAR SYNTH</div>
            ...
            <div class="guitar-panel">
                <div class="guitar-row">
                    <div style="display:flex; flex-direction:column; gap:8px;">
                        ...
                        <input type="file" id="guitar-wav-upload" accept="audio/wav,audio/mp3" style="display:none;">
                        <button class="btn-neon" id="btn-load-guitar" style="font-size:9px;">LOAD CUSTOM WAV TO STYLE</button>
                    </div>

                    <div style="display:flex; gap:10px;" id="guitar-knobs"></div>
                    
                    <div style="display:flex; flex-direction:column; gap:5px; justify-content:center;">
                        <button class="btn-neon" id="btn-toggle-keys">SHOW KEYS</button>
                        <div style="font-size: 8px; color: #888; text-align:center;">USE '4' AND '5'<br>TO SHIFT STRINGS</div>
                    </div>
                </div>

                <div class="fretboard-container" id="guitar-fretboard">
                    <!-- Generoidaan JS:llä -->
                </div>
            </div>
        `;
        ...
        const fileInput = containerElement.querySelector('#guitar-wav-upload');
        containerElement.querySelector('#btn-load-guitar').addEventListener('click', () => fileInput.click());
        ...
        const fretboardDiv = containerElement.querySelector('#guitar-fretboard');
        ...
        for (let s = 0; s < 6; s++) { 
            ...
            for (let f = 0; f < 5; f++) {
                ...
                fretCell.className = 'guitar-fret';
        ...
        const knobContainer = containerElement.querySelector('#guitar-knobs');
