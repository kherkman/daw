// bassamp.js
window.CustomAudioEffect = class BassAmpEffect {
    constructor(audioCtx) {
        ...
        this.midEQ.frequency.value = 500;
        ...
        // Reititys: Input -> Low -> Mid -> High -> Drive -> Volume -> Output
        this.input.connect(this.lowEQ);
        this.lowEQ.connect(this.midEQ);
        this.midEQ.connect(this.highEQ);
        this.highEQ.connect(this.driveNode);
    ...
    makeDistortionCurve(amount) {
        ...
        for (let i = 0; i < n_samples; ++i) {
            ...
            // Bassolle tyypillinen asymmetrinen särö tai putkimainen saturaatio
    ...
    renderUI(containerElement) {
        ...
        containerElement.innerHTML = `
            <div style="margin-bottom: 5px; color: var(--fx-color); font-weight: bold; text-align: center; letter-spacing: 3px; font-size: 14px;">BASS AMP</div>
            <div style="display: flex; flex-wrap: wrap; justify-content: center; align-items: center; padding: 15px 0; gap: 20px;" id="bassamp-dashboard"></div>
        `;
        const dashboard = containerElement.querySelector('#bassamp-dashboard');
